...
export const login = async (req, res) => {
  ...
  try {
    const query = 'SELECT id, name, email, password, role, isFirstLogin FROM users WHERE email = ?';
    db.query(query, [email], async (err, results) => {
      ...
      const token = jwt.sign({ id: user.id, email: user.email, role: user.role, isFirstLogin: user.isFirstLogin }, JWT_SECRET, { expiresIn: '24h' });
